...
describe('mcpSetupTemplates', () => {
  ...
  it('renders a claude code CLI command with the /mcp url and bearer header', () => {
    const target = MCP_SETUP_TARGETS.find((t) => t.id === 'claude-code-cli')!;
    const snippet = target.render({ apiBaseUrl: 'http://localhost:8787', origin: '', clientToken: 'mcp_x.y' });
    expect(snippet).toContain('claude mcp add --transport http');
    expect(snippet).toContain('http://localhost:8787/mcp');
