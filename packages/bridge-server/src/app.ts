...
import { BraveKeyPool } from './brave/keyPool.js';
import { RotatingBraveClient } from './brave/rotatingClient.js';
...
export function createBridgeApp(options: CreateBridgeAppOptions = {}): express.Express {
  ...
  // Brave key pool setup
  const bravePool = new BraveKeyPool({
    prisma,
    encryptionKey,
    getSelectionStrategy: () => serverSettings.getTavilyKeySelectionStrategy()
  });
  const braveClient = new RotatingBraveClient({
    pool: bravePool,
    prisma,
    maxRetries: MAX_RETRIES
  });
  ...
  app.post('/mcp', asyncHandler(async (req: any, res: any) => {
    ...
    if (!validated.ok) {
      const errorMsg = validated.error;
    ...
    if (!globalCheck.ok) {
      const retryAfter = globalCheck.retryAfterMs;
